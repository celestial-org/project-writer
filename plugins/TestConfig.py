...
def run_lite_command(c, m):
  ...
  for url in urls:
    ...
    if count is None:
      ...
      return
    stt = m.reply(f'**{m.from_user.first_name}** vừa bắt đầu đợt kiểm tra mới đến liên kết {url}\nSố lượng cấu hình: **{count}**\nSố lần kiểm tra: **{ping}**\n```Lưu ý:\nSố lần kiểm tra cao có thể gây quá tải hệ thống và không trả về kết quả như mong đợi\n```', quote=True)
    cmd = f"./lite -ping {ping} -config config.json -test {test_url}"
    os.system(cmd)
...
def run_lite_command_ping(c, m):
  ...
  for url in urls:
    ...
    if count is None:
      ...
      return
    stt = m.reply(f'**{m.from_user.first_name}** vừa bắt đầu đợt kiểm tra mới đến liên kết {url}\nSố lượng cấu hình: **{count}**\nSố lần kiểm tra: **{ping}**\n```Lưu ý:\nSố lần kiểm tra cao có thể gây quá tải hệ thống và không trả về kết quả như mong đợi\n```', quote=True)
    cmd = f"./lite -ping {ping} -config pingonly.json -test {test_url}"
    os.system(cmd)
    m.reply_photo(photo='out.png', quote=True, caption=f"```sponsor\nTran Han Thang\n```\n**{m.from_user.first_name}**", reply_markup=InlineKeyboardMarkup(
    [[InlineKeyboardButton("Subscription", url=url)]]))
    os.system('rm out.png')
...
def run_lite_command_speed(c, m):
  url_pattern = re.compile(r'((http[s]?|vmess|trojan|vless|ss)://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+)')
  ping = 1
  if len(m.command) > 1 and m.command[1].isdigit():
    ping = int(m.command[1])
  ...
  for url in urls:
    ...
    if count is None:
      ...
      return
    stt = m.reply(f'**{m.from_user.first_name}** vừa bắt đầu đợt kiểm tra mới đến liên kết {url}\nSố lượng cấu hình: **{count}**\nSố lần kiểm tra: **{ping}**\n```Lưu ý:\nSố lần kiểm tra cao có thể gây quá tải hệ thống và không trả về kết quả như mong đợi\n```', quote=True)
    cmd = f"./lite -ping {ping} -config speedonly.json -test {test_url}"
    os.system(cmd)
    m.reply_photo(photo='out.png', quote=True, caption=f"```sponsor\nTran Han Thang\n```\n**{m.from_user.first_name}**", reply_markup=InlineKeyboardMarkup(
    [[InlineKeyboardButton("Subscription", url=url)]]))
    os.system('rm out.png')
