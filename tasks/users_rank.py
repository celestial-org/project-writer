...
def counter(c, m):
    if m.from_user:
        ...
        if result:
            ...
            cursor.execute("UPDATE user_ranks SET count = %s WHERE user_key = %s", (count, key))
        else:
            count = 1
            cursor.execute("INSERT INTO user_ranks (user_key, count) VALUES (%s, %s)", (key, count))
        conn.commit()

@app.on_message(filters.command("rank") & filters.chat("share_v2ray_file"))
def get_rank(c, m):
    m.reply_chat_action(ChatAction.TYPING)
    cursor.execute("SELECT user_key, count FROM user_ranks ORDER BY count DESC")
    rows = cursor.fetchall()
    users = [
        f"{i + 1}) **{row[0]} ({row[1]})**" if i < 4 else f"{i + 1}) {row[0]} ({row[1]})"
